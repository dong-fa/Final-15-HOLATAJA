...
interface QuantityProps {
  ...
  onChange: (value: number) => void; // 수량이 변경될 때 호출되는 함수
}

export default function QuantityCount({ quantity, onChange }: QuantityProps) {
  // 마이너스 버튼 클릭 시 실행되는 함수
  const handleMinus = () => {
    if (quantity > 1) { // 수량이 1보다 클 때만 감소
      onChange(quantity - 1); // 수량을 1 감소시켜 부모로 전달
    }
  };

  // 플러스 버튼 클릭 시 실행되는 함수
  const handlePlus = () => {
    onChange(quantity + 1); // 수량을 1 증가시켜 부모로 전달
  };

  return (
    <div className="flex items-center gap-4"> {/* 버튼과 수량을 가로로 정렬, 간격 4 */}
      <button
