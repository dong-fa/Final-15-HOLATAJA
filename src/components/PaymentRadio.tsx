'use client'; // 이 파일이 브라우저에서 동작하는 클라이언트 컴포넌트임을 명시

import Image from 'next/image'; // next/image를 사용하면 이미지를 최적화해서 보여줄 수 있음
import { PaymentOption } from '../types/payment'; // 결제 옵션의 타입을 불러옴

// PaymentRadioProps 타입 정의
// options: 결제 수단 목록 배열
// selected: 현재 선택된 결제 수단의 id (문자열)
// onSelectOption: 결제 수단을 선택했을 때 실행되는 함수 (id를 인자로 받음)
      ...
      {options.map(option => (
        ...
        <label key={option.id} className="flex items-center cursor-pointer">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* option.width와 option.height가 있으면 그 값을, 없으면 기본값 50, 20 사용 */}
            <Image src={option.logo} alt={option.label} width={option.width ?? 50} height={option.height ?? 20} />
            {/* 결제 수단 이름. 스타일 적용 */}
